...
ORDER BY id DESC");
    ...
    public function warehouse()
    {
        ...
        $query = $this->db->query("SELECT c.title,c.id,count(distinct pid) pc,SUM(product_price*s.qty) salessum,SUM(fproduct_price*s.qty) worthsum,SUM(s.qty) qty FROM geopos_warehouse AS c "
                . "LEFT JOIN tb_stock s on c.id=s.warehouse_id "
                . "LEFT JOIN geopos_products p on s.product_id=p.pid and p.i_stock=1 "
                . " $where group by c.title,c.id");
        return $query->result_array();
    }

...
class Products extends CI_Controller
{
    ...
    public function product_list()
    {
        ...
        if ($catid > 0) {
            $list = $this->products->get_datatables_item($catid, '', $sub);
        } else {
            $list = $this->products->get_datatables_item();
        }
        $data = array();
        ...
        foreach ($list as $prd) {
            ...
            $row[] = $no;
            $pid = $prd->pid;
            $row[] = '<a href="#" data-object-id="' . $pid . '" class="view-object">' . $prd->product_name . '<br><span class="avatar-lg align-baseline"><img src="' . base_url() . 'userfiles/product/thumbnail/' . $prd->image . '" ></span></a>';
            //$row[] = +$prd->qty;
...
&nbsp;<a href="' . base_url() . 'products/edit?id=' . $pid . '"  class="btn btn-purple btn-sm"><span class="fa fa-edit"></span>' . $this->lang->line('Edit') . '</a><div class="dropdown-divider"></div>&nbsp;<a href="#" data-object-id="' . $pid . '" class="btn btn-danger btn-sm  delete-object"><span class="fa fa-trash"></span>' . $this->lang->line('Delete') . '</a>
    ...
    public function addproduct()
    {
        ...
        $is_stock = $this->input->post('chk_stock_item');
        $wdate = datefordatabase($this->input->post('wdate'));
        $code_type = $this->input->post('code_type');
        $sub_cat = $this->input->post('sub_cat');
        $brand = $this->input->post('brand');
        if ($catid) {
            $this->products->addnew($catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $v_type, $v_stock, $v_alert, $wdate, $code_type, $w_type, $w_stock, $w_alert, $sub_cat, $brand,$is_stock);
        }
    }
    ...
    public function editproduct()
    {
        ...
        $brand = $this->input->post('brand');
        if ($pid) {
            $this->products->edit($pid, $catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $code_type, $sub_cat, $brand,$is_stock);
        }
    }
    ...
    public function stock_transfer()
    {
        if ($this->input->post()) {
            ...
            $unique_id = $this->input->post('product_unique_id');
            $transferdate = $this->input->post('transferdate');
            $notes = $this->input->post('notes');
            $from_warehouse = $this->input->post('from_warehouse');
            $to_warehouse = $this->input->post('to_warehouse');
            if($from_warehouse==0){
                echo json_encode(array('status' => 'Error', 'message' => "Please select transfer out warehouse"));
                exit;
            }
            if($to_warehouse==0){
                echo json_encode(array('status' => 'Error', 'message' => "Please select transfer in warehouse"));
                exit;
            }
            if($from_warehouse==$to_warehouse){
                echo json_encode(array('status' => 'Error', 'message' => "Please select different warehouse"));
                exit;
            }
            $proindex=0;
            ...
            if($proindex==0){
                echo json_encode(array('status' => 'Error', 
                    'message' => "Please choose product from product list. Go to Item manager section if you have not added the products."));
                exit;
            }
            $this->products->transfer($from_warehouse, $to_warehouse, $transferdate, $notes, $proindex, $pid, $psid, $unique_id, $qty, $pro_name);
        } else {
            $data['cat'] = $this->categories_model->category_list();
            $data['warehouse'] = $this->categories_model->warehouse_list();
    ...
    public function stock_transfer_list(){
        $head['title'] = "Product Transfer History";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('products/transfer_list');
        $this->load->view('fixed/footer');
    }
    
    public function ajax_transfer_list()
    {
        $list = $this->products->get_transfer_datatables($this->limited);
        $data = array();
        $no = $this->input->post('start');
        $i=0;
        foreach ($list as $invoices) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = dateformat($invoices->transferdate);
            $row[] = number_format($invoices->items);
            $row[] = $invoices->fwarehouse;
            $row[] = $invoices->twarehouse;
            $row[] = '<a href="' . site_url("products/transfers_view?id=$invoices->id") . '" class="btn btn-success btn-sm view-object" title="View"><i class="fa fa-eye"></i></a>';
            $data[] = $row;
        }
        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsTotal" => $this->products->count_all_transfer($this->limited),
            "recordsFiltered" => $i,
            "data" => $data,
        );
        //output to json format
    ...
    public function transfers_view(){
        $id = $this->input->get("id");
        $data['transfer'] = $this->products->get_transfer_master($id);
        $data['transfer_item'] = $this->products->get_transfer_detail($id);
        $this->load->view('products/view-transfer',$data);
    }
    ...
    public function stock_adjustment()
    {
        if ($this->input->post()) {
            ...
            $qty = $this->input->post('product_qty');
            $unique_id = $this->input->post('product_unique_id');
            $adjustdate = $this->input->post('adjustdate');
            $notes = $this->input->post('notes');
            $from_warehouse = $this->input->post('from_warehouse');
            if($from_warehouse==0){
                echo json_encode(array('status' => 'Error', 'message' => "Please select warehouse"));
                exit;
            }
            $proindex=0;
            foreach ($pid as $key => $value) {
            ...
            $this->products->adjust($from_warehouse, $adjustdate, $notes, $proindex, $pid, $psid, $unique_id, $qty, $pro_name);
        } else {
            $data['warehouse'] = $this->categories_model->warehouse_list();
            $head['title'] = "Stock Adjustment";
            $head['usernm'] = $this->aauth->get_user()->username;
            $this->load->view('fixed/header', $head);
            $this->load->view('products/stock_adjustment', $data);
            $this->load->view('fixed/footer');
        }
    }
    
    public function reverse_adjustment($id){
        $this->products->reverse_adjustment($id);
        redirect("products/stock_adjustment_list");
    }
    
    public function stock_adjustment_list(){
        $head['title'] = "Product Adjust History";
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('products/adjustment_list');
        $this->load->view('fixed/footer');
    }
    
    public function ajax_adjustment_list()
    {
        $list = $this->products->get_adjustment_datatables($this->limited);
        $data = array();
        $no = $this->input->post('start');
        $i=0;
        foreach ($list as $invoices) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = dateformat($invoices->adjustdate);
            $row[] = number_format($invoices->items);
            $row[] = $invoices->fwarehouse;
            $row[] = $invoices->total_cost;
            $row[] = $invoices->status=='success'?"<span class='st-partial'>Success</span>":"<span class='st-canceled'>Canceled</span>";
            $row[] = '<a href="' . site_url("products/adjustment_view?id=$invoices->id") . '" class="btn btn-success btn-sm view-object" title="View"><i class="fa fa-eye"></i></a>'
                    ."&nbsp;".($invoices->status=='success'?"<a href='#' data-object-id-1='$invoices->id' class='btn btn-danger btn-sm delete-object'><span class='fa fa-trash'></span></a>":"");
            $data[] = $row;
        }
        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsTotal" => $this->products->count_all_adjustment($this->limited),
            "recordsFiltered" => $i,
            "data" => $data,
        );
        //output to json format
    ...
    public function adjustment_view(){
        $id = $this->input->get("id");
        $data['transfer'] = $this->products->get_adjustment_master($id);
        $data['transfer_item'] = $this->products->get_adjustment_detail($id);
        $this->load->view('products/view-adjustment',$data);
    }
    ...
    public function view_over()
    {
        ...
        $this->db->select('geopos_products.*,geopos_warehouse.title,tb_stock.qty as product_qty,tb_stock.unique_id');
        $this->db->from('geopos_products');
        $this->db->where('geopos_products.pid', $pid);
        $this->db->join('tb_stock', 'geopos_products.pid = tb_stock.product_id',"LEFT");
        $this->db->join('geopos_warehouse', 'tb_stock.warehouse_id = geopos_warehouse.id',"LEFT");
        ...
        $this->db->where('geopos_products.i_stock', 1);
        $this->db->where('tb_stock.qty>0');
        $this->db->order_by('tb_stock.warehouse_id');

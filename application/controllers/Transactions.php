...
class Transactions extends CI_Controller
{
    ...
    public function cancelinvoice()
    {
        ...
        //restock
        $warehouse_name = $this->invoice->get_invoice_warehouse_name($tid);
        $purchase_product = $this->invoice->invoice_products_stock($tid);
        ...
        foreach($purchase_product as $p){
            ...
            $pro_update++;
            $this->aauth->applog("[Product Sale Cancel] ".$p['product']." Qty +".$p['qty']." to $warehouse_name", $this->aauth->get_user()->username);
        }
        if($pro_update>0){
    ...
    public function cancelpurchase()
    {
        ...
        //restock
        $warehouse_name = $this->purchase->purchase_products_stock($tid);
        $purchase_product = $this->purchase->purchase_products_stock($tid);
        $data_update_stock = array();
        $pro_update = 0;
        foreach($purchase_product as $p){
            $d_update = array(
            ...
            $data_update_stock[$pro_update] = $d_update;
            $pro_update++;
            $this->aauth->applog("[Product Purchase Cancel] ".$p['product']." Qty -".$p['qty']." from $warehouse_name", $this->aauth->get_user()->username);
        }
        if($pro_update>0){
            $this->db->update_batch('tb_stock', $data_update_stock,'id');
        }

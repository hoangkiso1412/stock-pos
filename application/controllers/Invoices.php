...
class Invoices extends CI_Controller
{
    ...
    public function create()
    {
        ...
        $data['lastinvoice'] = $this->invoices->lastinvoice();
    ...
    public function edit()
    {
        ...
        if ($data['invoice']['id']) $data['products'] = $this->invoices->invoice_products_stock($tid);
    ...
    public function action()
    {
        ...
        $warehouse = $this->input->post('s_warehouses');
        ...
        if ($this->db->insert('geopos_invoices', $data)) {
            ...
            foreach ($pid as $key => $value) {
                ...
                $this->aauth->applog("[Product Sale] $product_name1[$key] Qty $product_qty[$key] From $warehouse_name", $this->aauth->get_user()->username);
...
//                }
            ...
            if ($prodindex > 0) {
                ...
                $purchase_product = $this->invoices->invoice_products_stock($invocieno);
                $data_update = array();
                $pro_update = 0;
...
// Your Account SID and Auth Token from twilio.com/console
    ...
    {
        $list = $this->invoices->get_datatables($this->limited);
        $data = array();
        ...
        foreach ($list as $invoices) {
            ...
            $row[] = '<a href="' . base_url("invoices/view?id=$invoices->id") . '" class="btn btn-success btn-sm" title="View"><i class="fa fa-eye"></i></a>&nbsp;<a href="' . base_url("invoices/printinvoice?id=$invoices->id") . '&d=1" class="btn btn-info btn-sm"  title="Download"><span class="fa fa-download"></span></a> <a href="#" data-object-id-1="' . $invoices->id . '" class="btn btn-danger btn-sm delete-object hidden"><span class="fa fa-trash"></span></a>';
            $data[] = $row;
        }
        $output = array(
            "draw" => $this->input->post('draw'),
            "recordsTotal" => $this->invoices->count_all($this->limited),
            "recordsFiltered" => $this->invoices->count_filtered($this->limited),
            "data" => $data,
        );
    ...
    public function view()
    {
        ...
        $data['invoice'] = $this->invoices->invoice_details($tid, $this->limited);
        $data['attach'] = $this->invoices->attach($tid);
        $data['c_custom_fields'] = $this->custom->view_fields_data($data['invoice']['cid'], 1);
        $head['usernm'] = $this->aauth->get_user()->username;
        $head['title'] = "Invoice " . $data['invoice']['tid'];
        $this->load->view('fixed/header', $head);
        $data['products'] = $this->invoices->invoice_products_stock($tid);
        if ($data['invoice']['id']) $data['activity'] = $this->invoices->invoice_transactions($tid);
        $data['employee'] = $this->invoices->employee($data['invoice']['eid']);
        if ($data['invoice']['id']) {
            $data['invoice']['id'] = $tid;
    ...
    public function delete_i()
    {
        if ($this->aauth->premission(11)) {
            ...
            if ($this->invoices->invoice_delete($id, $this->limited)) {
                echo json_encode(array('status' => 'Success', 'message' =>
    ...
    public function editaction()
    {
        ...
        $i = 0;
        if ($this->limited) {
            $employee = $this->invoices->invoice_details($iid, $this->limited);
        ...
        if ($this->db->update('geopos_invoices', $data)) {
            ...
            $purchase_product = $this->invoices->invoice_products_stock($iid);
            $data_update = array();
            $pro_update = 0;
            ...
            foreach($purchase_product as $p){
                if($p['stock_id']!=0){
                    ...
                    $data_update[$pro_update] = $d_update;
                    $pro_update++;
                    $this->aauth->applog("[Product Sale Edit] ".$p['product']." Qty +".$p['qty']." To $warehouse_name", $this->aauth->get_user()->username);
                }
            }
            if($pro_update>0){
                $this->db->update_batch('tb_stock', $data_update,'id');
            ...
            if ($prodindex > 0) {
                ...
                foreach($purchase_product as $p){
                    if($p['stock_id']!=0){
                        ...
                        $data_update[$pro_update] = $d_update;
                        $pro_update++;
                        $this->aauth->applog("[Product Sale Edit] ".$p['product']." Qty -".$p['qty']." From $warehouse_name", $this->aauth->get_user()->username);
                    }
                }
                if($pro_update>0){
                    $this->db->update_batch('tb_stock', $data_update,'id');
    ...
    public function file_handling()
    {
        if ($this->input->get('op')) {
            ...
            if ($this->invoices->meta_delete($invoice, 1, $name)) {
            ...
            if ($files != '') {
                ...
                $this->invoices->meta_insert($id, 1, $files);
            }
        }
...
<address>' . $this->config->item('address') . '<br>' . $this->config->item('address2') . '</address>
    ...
    public function view_payslip()
    {
        ...
        $data['invoice'] = $this->invoices->invoice_details($inv, $this->limited);

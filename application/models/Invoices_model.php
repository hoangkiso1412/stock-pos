...
class Invoices_model extends CI_Model
{
    ...
    {
        $this->db->select("p.*,ifnull(s.id,0) as stock_id,ifnull(s.qty,0) as stock_qty,ifnull(s.sale_status,'in-stock') sale_status,ifnull(s.init_stock,0) init_stock,s.product_desc stock_desc,pr.i_stock");
        $this->db->from('geopos_invoice_items p');
        $this->db->join('tb_stock s',"p.pid=s.product_id and p.warehouse_id=s.warehouse_id and ifnull(p.product_unique_id,'')=ifnull(s.unique_id,'')","left");
        $this->db->join('geopos_products pr',"p.pid=pr.pid","left");
        $this->db->where('p.tid', $id);
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function get_invoice_warehouse_name($id=0){
        
        $this->db->select('warehouse_id');
        $this->db->from('geopos_invoices');
        $this->db->where('id', $id);
        $query = $this->db->get();
        $warehouse = $query->row_array()['warehouse_id'];

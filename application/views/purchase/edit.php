                        ...
                        <div class="col-sm-6 cmp-pnl">
                            ...
                            <div class="clientinfo">
                                ...
                                <div type="text" id="customer_phone">Phone: <strong>' . $invoice['phone'] . '</strong><br>Email: <strong>' . $invoice['email'] . '</strong></div>
                                        ...
                                        <hr><?php echo $this->lang->line('Warehouse') ?> : <b><?php echo $warehouse_name ?></b>
                                        <input type='hidden' id="s_warehouses" class="selectpicker form-control" name="s_warehouse" value="<?php echo $invoice['warehouse_id'] ?>" >
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6 cmp-pnl">
                            ...
                            <table class="table-responsive tfr my_stripe">
                                ...
                                <thead>
                                    <tr class="item_header bg-gradient-directional-amber">
                                        <th width="19%" class="text-center"><?php echo $this->lang->line('Item Name') ?></th>
                                        <th width="19%" class="text-center"><?php echo $this->lang->line('Unique ID') ?></th>
                                        <th width="1%" ></th>
                                    ...
                                    <?php echo $this->lang->line('Amount') ?>
                                        ...
                                        <th width="4%" class="text-center"><?php echo $this->lang->line('Action') ?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                <?php $i = 0;
                                    ...
                                    <tr>
                                        <td>
                                            <input type="text" class="form-control" name="product_name[]" 
                                                   ...
                                                   id='productname_purchase-<?php echo $i?>' value="<?php echo $row['product']?>" readonly>
                                        </td>
                                        <td><input type="text" class="form-control" name="product_unique_id[]"
                                                   placeholder="Unique ID" value="<?php echo $row['product_unique_id'] ?>"
                                                   id='product_unique_id-<?php echo $i?>' data-role="purchase_unique_key" data-key="uniquekey" autocomplete="off" readonly>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <input type="text" class="form-control req amnt" name="product_qty[]" id="amount-<?php echo $i?>"
                                                   ...
                                                   autocomplete="off" value="<?php echo amountFormat_general($row['qty'])?>" data-key="qty">
                                            <input type="hidden" id="old-amount-<?php echo $i?>" name="old_product_qty[]" value="<?php echo amountFormat_general($row['qty'])?>" >
                                        </td>
                                        <td>
                                            <input type="text" class="form-control req prc" name="product_price[]" id="price-<?php echo $i?>"
                                                   onkeypress="return isNumber(event)" onkeyup="rowTotal('<?php echo $i?>'), billUpyog()"
                                                   autocomplete="off" value="<?php echo edit_amountExchange_s($row['price'], $invoice['multi'], $this->aauth->get_user()->loc)?>"></td>
                                        <td> <input type="text" class="form-control vat" name="product_tax[]" id="vat-<?php echo $i?>"
                                                    onkeypress="return isNumber(event)" onkeyup="rowTotal('<?php echo $i?>'), billUpyog()"
                                                    autocomplete="off"  value="<?php echo amountFormat_general($row['tax'])?>"></td>
                                        ...
                                        <td><span class="currenty"><?php echo $this->config->item('currency')?></span>
                                            <strong><span class="ttlText" id="result-<?php echo $i?>"><?php echo edit_amountExchange_s($row['subtotal'], $invoice['multi'], $this->aauth->get_user()->loc)?></span></strong></td>
                                        <td class="text-center">
                                            ...
                                            if($row['product_unique_id']==''){
                                                ...
                                                if($row['sale_status']=='in-stock'&&$row['sale_status']!='wasted'&&$row['init_stock']==$row['warehouse_id']){
                                                    $btn_remove = true;
                                                }
                                                else{
                                                    $btn_remove = false;
                                            ...
                                            if($btn_remove){
                                                ?>
                                            <button type="button" data-rowid="<?php echo $i?>" class="btn btn-danger removeProdPur" title="Remove"> <i class="fa fa-minus-square"></i> </button>
                                            <?php
                                            }
                                            ?>
                                ...
                                <input type="hidden" name="psid[]" id="psid-<?php echo $i?>" value="<?php echo $row['stock_id']?>">
                                <input type="hidden" class="qtyIn" name="qtyIn[]" id="qtyIn-<?php echo $i?>">
                                <input type="hidden" class="puid" name="puid[]" id="puid-<?php echo $i?>" value="<?php echo $row['id'] ?>">
                                </tr>
                                <tr class="desc_p">
                                    <td colspan="10">
                                        <textarea id="dpid-<?php echo $i?>" class="form-control" name="product_description[]" placeholder="Enter Product description" autocomplete="off"><?php $row['product_des'] ?></textarea><br>
                                    </td>
                                </tr>
                                <?php
                                $i++;
                                ...
                                <tr class="last-item-row sub_c">
                                    <td class="add-row">
                                        <button type="button" class="btn btn-success" id="addproductpurchase">
                                ...
                                <tr class="sub_c" style="display: table-row;">
                                    <td colspan="8" align="right"><input type="hidden"

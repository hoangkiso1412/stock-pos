...
$("#submit-data").on("click", function (e) {
    e.preventDefault();
    var invalid = false;
    $("#saman-row").find('i[id^=product_check]').each(function(e){
        if($(this).hasClass('icon-close')||$(this).hasClass('text-danger')){
            invalid = true;
        }
    });
    if(!invalid){
        ...
        $("#notify").removeClass("alert-success").addClass("alert-warning").fadeIn();
        $("#notify .message").html("<strong>Error</strong>: It appears you have enter invalid information of product!");
        $("html, body").scrollTop($("body").offset().top);
    }
});
    ...
    if (errorNum > 0) {
        ...
        jQuery.ajax({
            ...
            error: function (data,xhr, status) {
                console.log(data);
                $("#notify .message").html("<strong>" + data.status + "</strong>: " + data.statusText);
                $("#notify").removeClass("alert-success").addClass("alert-warning").fadeIn();
                $("html, body").scrollTop($("body").offset().top);
            }
        });

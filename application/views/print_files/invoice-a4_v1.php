...
<html>
    ...
    <body dir="<?= LTR ?>">
        ...
        <?php } ?>
            ...
            <?php if ($invoice['tax'] > 0) echo '<td class="t_center">' . $this->lang->line('Tax') . '</td>';
            if ($invoice['discount'] > 0) echo '<td class="t_center">' . $this->lang->line('Discount') . '</td>'; ?>
                    <td class="t_center">
        ...
        foreach ($products as $row) {
            ...
            echo '<tr class="item' . $flag . '">  <td>' . $n . '</td>
                            <td>' . $row['product'] . '</td>
							<td style="width:12%;" class="t_center">' . amountExchange($row['price'], $invoice['multi'], $invoice['loc']) . '</td>
                            <td style="width:12%;" class="t_center">' . +$row['qty'] . $row['unit'] . '</td>   ';
            if ($invoice['tax'] > 0) {
                $cols++;
                echo '<td style="width:16%;" class="t_center">' . amountExchange($row['totaltax'], $invoice['multi'], $invoice['loc']) . ' <span class="tax">(' . amountFormat_s($row['tax']) . '%)</span></td>';
            }
            if ($invoice['discount'] > 0) {
                $cols++;
                echo ' <td style="width:16%;" class="t_center">' . amountExchange($row['totaldiscount'], $invoice['multi'], $invoice['loc']) . '</td>';
            }
            echo '<td class="t_center">' . amountExchange($row['subtotal'], $invoice['multi'], $invoice['loc']) . '</td></tr>';
                
            if ($row['product_des']||($emp&&$row['product_unique_id'])) {
                $cc = $cols++;
                    
                echo '<tr class="item' . $flag . ' descr">  <td> </td>
                            <td colspan="' . $cc . '">' .($row['product_unique_id']&&$emp? 'Key: '.$row['product_unique_id'].'<br>':'') . nl2br($row['product_des']) . '&nbsp;</td>
        ...
        ?>
            ...
            <table class="subtotal">
                ...
                <tr class="f_summary">
                    ...
                    <td class="t_center"><?php echo amountExchange($sub_t, $invoice['multi'], $invoice['loc']); ?></td>
                </tr>
        <?php if ($invoice['tax'] > 0) {
            echo '<tr>
            <td> ' . $this->lang->line('Total Tax') . ' :</td>
            <td class="t_center">' . amountExchange($invoice['tax'], $invoice['multi'], $invoice['loc']) . '</td>
        </tr>';
        }
        if ($invoice['discount'] > 0) {
            echo '<tr>
            <td>' . $this->lang->line('Total Discount') . ':</td>
            <td class="t_center">' . amountExchange($invoice['discount'], $invoice['multi'], $invoice['loc']) . '</td>
        </tr>';
        }
        if ($invoice['shipping'] > 0) {
            echo '<tr>
            <td>' . $this->lang->line('Shipping') . ':</td>
            <td class="t_center">' . amountExchange($invoice['shipping'], $invoice['multi'], $invoice['loc']) . '</td>
        </tr>';
        }
        ?>
                <tr>
                    <td><?php echo $this->lang->line('Balance Due') ?>:</td>
                    <td class="t_center"><strong><?php $rming = $invoice['total'] - $invoice['pamnt'];
    if ($rming < 0) {
        $rming = 0;
    }
    if (@$round_off['other']) {
    ...
    <table border="0">
        <tr>
            <td width="50%">
                <br>
                <center><div class="sign2"><?php echo $this->lang->line('Customer')?></div></center>
            </td>
            <td>
                <center>
                    <br>
                    <div class="sign2"><?php echo $this->lang->line('Seller')?></div>
                    <div class="sign1">
                        <?php
                        if((int)$this->input->get('no')==1)
                        {
                            echo "<br><br><br>";
                        }
                        else{
                    ...
                    <div class="sign2"><?php echo $employee['name'] ?></div>
                </center>
            </td>
        </tr>
    </table>
    <div class="terms"><?php echo $invoice['notes']?>
        <hr>
        <strong><?php echo $this->lang->line('Terms')?>:</strong><br>
        <strong><?php echo $invoice['termtit']?></strong><br><?php echo $invoice['terms']?>
    </div>
    </div>
</body>
</html>
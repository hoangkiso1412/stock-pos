                    ...
                    <div class="col">
                        ...
                        if ($invoice['status'] != 'canceled') { ?>
                            <div class="row">
                                ...
                                <div class="col-md-8">
                                    <div class="form-group mt-2 hidden" id="pmnt_div"><?php echo $this->lang->line('Payment') ?>:
                                    ...
                                    <div id="url" style="color:#fff"><?php echo base_url(uri_string()."?id=".$this->input->get("id")."&token=".$this->input->get("token"))?></div>
                                </div>
                                    ...
                                    <div class="btn-group mt-2">
                                        <button type="button" class="btn btn-primary" id="btn-copy" 
                                                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
                                                 data-clipboard-action="copy" data-clipboard-target="#url">
...
<th scope="row">' . $c . '</th>
                            ...
                            <td>' . amountExchange($row['subtotal'], $invoice['multi'], $invoice['loc']) . '</td>
                                    ...
                                    echo '<tr><td colspan=9>' . nl2br($row['product_des']) . '</td></tr>';
                                    if (CUSTOM) {
...
<th scope="row">' . $c . '</th>
                            ...
                            <td>' . amountExchange($row['subtotal'], $invoice['multi'], $invoice['loc']) . '</td>
                                        ...
                                        echo '<tr><td colspan=8>' . nl2br($row['product_des']) . '</td></tr>';
                                        if (CUSTOM) {
                                            $p_custom_fields = $this->custom->view_fields_data($row['pid'], 4, 1);
...
<th scope="row">' . $c . '</th>
                            ...
                            <td>' . amountExchange($row['subtotal'], $invoice['multi'], $invoice['loc']) . '</td>
                                        ...
                                        echo '<tr><td colspan=7>' . nl2br($row['product_des']) . '</td></tr>';
                                        if (CUSTOM) {
                                            $p_custom_fields = $this->custom->view_fields_data($row['pid'], 4, 1);
